 ...
 *
 * 目录不存在时以 3 退出并说明原因 —— 不能用 0，否则「本轮根本没跑」会被当成通过。
 */
...
if (!fs.existsSync(path.join(installRoot, '@deepseek-ai', 'dsh-tools'))) {
  console.error(`跳过：找不到 ${installRoot}\\@deepseek-ai\\dsh-tools（用 DSH_NODE_MODULES 指向含 @deepseek-ai 的 node_modules）`)
  // 不能 exit 0：那会让「本轮根本没跑」被当成通过
  process.exit(3)
}
