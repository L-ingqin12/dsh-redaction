...
const FIELDS = [
  ...
  },
  {
    ...
    allowInfinity: true,
    description: '单次调用允许扫描（正则）/保留（strip 截断）的 UTF-8 字节上限；整数且 >= 1，或 Infinity（YAML `.inf`）表示不限。',
  },
  {
 ...
 * 部署安装目录里，**从插件目录不可达**（详见 README.zh.md §2.1）。
 *
 * Cordis 只要求 `Config["~standard"].validate(config)`（`cordis/lib/index.js:955-960`）。
 * 两条轨道**约束集合相同**（`FIELDS` 一张表 + `strip[]` 的 `maxChars` 整数约束），
 * 但**并非逐字节一致**：Schemastery 会补嵌套默认值、保留未知键、报错文案是英文；
 * 兜底轨道不补嵌套默认值、丢弃未知键、报错文案是中文。**用户实际拿到的是兜底那一套**
 * （profile 以 `link:` 安装时 schemastery 解析不到），所以 README 描述的行为以兜底为准。
 */
...
function buildSchemasteryConfig(z) {
  ...
  const StripRule = z.object({
    ...
    maxChars: z.number().step(1).min(1).description('可选：目标字符串超过该长度（Unicode 码位，整数 >= 1）时只保留这么长的头部并追加省略标记；不给则整个字段被丢弃。'),
  })
  const shape = {
...
function schemasteryField(z, field) {
  ...
  return schemasteryNumber(z, field).default(field.default).description(field.description)
}

/**
 * Schemastery 轨道的数值字段：**整数且 >= 1**（`step(1)` 就是"整数"约束）。
 *
 * 为什么 `allowInfinity` 的字段要再并一个 `const(Infinity)`：`z.number()` 本身接受
 * Infinity（本版本实测如此），但 `step(1)` 会把它判掉（`Infinity % 1` 是 NaN），
 * 而 `maxScannedBytes: .inf` 是"不限预算"的逃生口，必须保留。并入之后该字段的错误
 * 文案会退化成 `expected number | Infinity but got …`（仍然带字段路径），
 * 精确的中文文案由兜底轨道给出 —— 两轨道的差异见 README.zh.md §2.1。
 */
function schemasteryNumber(z, field) {
  const positiveInteger = z.number().step(1).min(1)
  return field.allowInfinity === true ? z.union([positiveInteger, z.const(Infinity)]) : positiveInteger
}
      ...
      validate(value) {
        ...
        for (const field of FIELDS) {
          ...
          if (field.type === 'number') {
            // 与 Schemastery 轨道同一条约束：整数且 >= 1；`allowInfinity` 的字段额外接受
            // Infinity（YAML `.inf`）。这样 0 / 1.5 / 负数 / 非数字 / NaN / -Infinity
            // 都在**配置校验阶段**被拒，而不是等到 apply() 里抛错（那会中止整个 profile 的 boot）。
            const expected = field.allowInfinity === true
              ? `${field.key} 必须是 >= 1 的整数，或 Infinity（表示不限）`
              : `${field.key} 必须是 >= 1 的整数`
            if (field.allowInfinity === true && given === Infinity) {
              out[field.key] = given
            } else if (!Number.isInteger(given) || given < 1) {
              issues.push({ message: expected, path: [field.key] })
            } else out[field.key] = given
            continue
          }
        ...
        // 注意：字段值类型不对时上面只记了 issue、没有写进 out，所以这里要防空。
        const rawStrip = Array.isArray(out.strip) ? out.strip : []
        for (let index = 0; index < rawStrip.length; index += 1) {
          ...
          if (rule.maxChars !== undefined && (!Number.isInteger(rule.maxChars) || rule.maxChars < 1)) {
            issues.push({ message: 'strip 规则的 maxChars 必须是 >= 1 的整数', path })
          }
        }
...
export function apply(ctx, config) {
   ...
   *
   * 只配 strip（没有正则 replace 规则）时，失败结果里没有任何可改写的东西：
   * 这里刻意**不**做那次空扫描（否则一段超大的失败 content 会在
   * `onBudgetExceeded: error` 下把原本的失败改写成 CONTENT_POLICY 错误，
   * 白白丢掉工具自己的错误信息）。meta 的丢弃行为保持不变。
   */
  function scrubFailure(exec, result, applicable, budget) {
    const contentReport = applicable.length === 0
      ? unchangedReport(result.content)
      : scrubContentBlocks(result.content, { rules: applicable, budget })
    const originalMessage = typeof result.error?.message === 'string' ? result.error.message : undefined
    const messageReport = originalMessage === undefined || applicable.length === 0
      ? undefined
...
/**
 * 解析并校验 `maxScannedBytes` —— **第二道防线**。
 *
 * 正常路径上配置已经在 `Config` 校验阶段被拒（两条轨道都要求"整数且 >= 1，或 Infinity"，
 * 见 `FIELDS[].allowInfinity` 与 `schemasteryNumber`），所以这里只有**直接调用 `apply()`、
 * 绕过 Config** 时才会抛错（测试里就是这么用的）。保留它是为了让"绕过 Config 也不静默"。
 */
function resolveMaxScannedBytes(value) {
  if (value === undefined) return DEFAULT_MAX_SCANNED_BYTES
  if (value === Infinity) return Infinity
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) {
    throw new Error(`content-policy: maxScannedBytes 必须是 >= 1 的整数或 Infinity（得到 ${String(value)}）`)
  }
  return value
}
...
/** "什么都不做"的空报告（形状与 scrubContentBlocks 一致，供零规则路径复用）。 */
function unchangedReport(value) {
  return { value, changed: false, matched: false, hits: [], totalHits: 0, pinned: 0, skipped: 0, unsafe: 0, truncated: false, scannedBytes: 0 }
}
